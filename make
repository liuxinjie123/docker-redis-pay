...
export container_name=pay-redis-dev;
export project_name=pay-redis;
export image_name=ubuntu-redis;
export ip=${pay_redis_ip};
...
mbt_rewrite;
devCreate() {
	if ! docker run -d --name $container_name --net aegis-bridge --ip $ip -p 6002:6379 $image_name > /dev/null; then
